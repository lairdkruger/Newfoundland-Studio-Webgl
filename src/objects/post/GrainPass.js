...
export function addGrainPass(webgl, options) {
    console.log(webgl)
    ...
    webgl.renderer.getSize(renderSize)

    grainPass.uniforms.uWidth.value = renderSize.x
    grainPass.uniforms.uHeight.value = renderSize.y

    grainPass.uniforms.uGrainAmount.value = 0.03
    grainPass.uniforms.timer.value = 0.0

    webgl.composer.addPass(grainPass)

    webgl.onUpdate((dt, time) => updateGrainUniforms(dt))

    function updateGrainUniforms(dt) {
        grainPass.uniforms.timer.value += dt
    }
}
    ...
    fragmentShader: `
        ...
        void main() 
        {
            ...
            if (colored)
            {
                ...
                noise.g = mix(noise.r,pnoise3D(vec3(rotCoordsG*vec2(uWidth/grainsize,uHeight/grainsize),1.0)),coloramount);
                noise.b = mix(noise.r,pnoise3D(vec3(rotCoordsB*vec2(uWidth/grainsize,uHeight/grainsize),2.0)),coloramount);
            }
